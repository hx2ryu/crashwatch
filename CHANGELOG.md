...
### Changed
- Workflow publishes via **npm Trusted Publishing (OIDC)**. Each of the 7 packages now has a Trusted Publisher configured on npmjs.com pointing at `hx2ryu/crashwatch`'s `release.yml`; `NODE_AUTH_TOKEN` dropped from the publish step. The 1-day bootstrap token used for `0.1.0-alpha.0` has been revoked and the `NPM_TOKEN` repo secret deleted. No long-lived credentials remain anywhere in the release path.
...
- **Scope decision: packages ship under `@hx2ryu/crashwatch-*`.** Every
  ...
  for the alpha. Affects 46 files: all 7 `package.json` names + peer/dep
  references, every TS import, READMEs, examples, JSON-schema examples,
  CLI init scaffolder. Build/typecheck/test (191 tests) green; dry-run
  tarballs clean post-rename. If the project graduates past single-owner,
  rename to a `@crashwatch` org is a one-shot `npm dist-tag` + republish
  under the new scope.
- **Semver-aware version comparison in the detector.** The prior-release
  ...
  exported from `@hx2ryu/crashwatch-core` for reuse in custom detectors. +9 core
  tests (47 total).
...
- **Pluggable detector via config.** `CrashwatchConfig` gained an optional
  ...
  type is exported from `@hx2ryu/crashwatch-core`. JSON Schema updated with a
...
- `@hx2ryu/crashwatch-tracker-github-issues` 0.1.0-alpha.0: GitHub Issues tracker
...
- `@hx2ryu/crashwatch-core` `defaultDetector` gained two new rules:
...
- Per-package READMEs for `@hx2ryu/crashwatch-core`, `@hx2ryu/crashwatch-cli`,
  `@hx2ryu/crashwatch-notifier-webhook`, and `@hx2ryu/crashwatch-notifier-slack`, matching
  the tone of the existing provider-firebase / provider-sentry READMEs.
- `@hx2ryu/crashwatch-provider-sentry` 0.1.0-alpha.0: Sentry provider backed by the
...
- `@hx2ryu/crashwatch-core` 0.1.0-alpha.0: types, config loader, JSONL store, default detector, plugin interfaces.
- `@hx2ryu/crashwatch-cli` 0.1.0-alpha.0: `init`, `validate`, `check` commands; `--config`, `--state`, `--dry-run`, `--json` options.
- `@hx2ryu/crashwatch-provider-firebase` 0.1.0-alpha.0: provider skeleton with `firebase-cli` and `bigquery` mode stubs.
- `@hx2ryu/crashwatch-notifier-webhook` 0.1.0-alpha.0: generic HTTP POST notifier with optional Slack Incoming-Webhook formatting.
- `@hx2ryu/crashwatch-notifier-slack` 0.1.0-alpha.0: convenience wrapper around the webhook notifier.
...
- `@hx2ryu/crashwatch-provider-firebase` now implements `listIssues`, `listEvents`, and
...
- `@hx2ryu/crashwatch-core`: 28 tests — `expandEnv`, `loadConfig` (YAML/JSON,
...
- `@hx2ryu/crashwatch-provider-firebase`: 37 tests — mapper shape tolerance,
...
- `@hx2ryu/crashwatch-cli`: end-to-end `check` smoke against fixture provider +
...
- `Issue` gained optional `recentEvents` / `recentImpactedUsers`: providers
  ...
  query. `@hx2ryu/crashwatch-provider-firebase` populates them from
